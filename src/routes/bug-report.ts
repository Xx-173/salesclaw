...
bugReportRoutes.post('/submit', authMiddleware, async (c) => {
  ...
  const fullBody = `${body}\n\n---\n> Submitted via SalesClaw by ${user.display_name || user.username}`;
  ...
  const url = `https://github.com/Xx-173/salesclaw/issues/new?title=${encodeURIComponent(title)}&body=${encodeURIComponent(truncatedBody)}`;
