...
function resolveDesktopUrls(): { serverUrl: string; rendererUrl: string } {
  const serverValue =
    ...
    process.env.SALESCLAW_SERVER_URL ||
    DEFAULT_SERVER_URL;
  const serverUrl = normalizeHttpUrl(serverValue, 'SalesClaw server URL');
  const rendererValue =
    getCliValue('--renderer-url') ||
    process.env.SALESCLAW_RENDERER_URL ||
    serverUrl;
  const rendererUrl = normalizeHttpUrl(rendererValue, 'SalesClaw renderer URL');
...
function errorPageHtml(failedUrl: string): string {
  ...
  return `<!doctype html>
<html><head><meta charset="utf-8"><title>SalesClaw is unavailable</title>
...
</head><body><main><h1>SalesClaw 后端未连接</h1><p>请先启动 SalesClaw 后端，或检查桌面应用配置的服务地址：</p><p><code>${safeUrl}</code></p><button id="retry">重试连接</button><script>document.getElementById('retry').addEventListener('click',()=>window.miniclawDesktop?.retry())</script></main></body></html>`;
}
...
function createApplicationMenu(): void {
  ...
  const template: MenuItemConstructorOptions[] = [
    ...(process.platform === 'darwin' ? [{ role: 'appMenu' as const }] : []),
    {
      label: 'SalesClaw',
      submenu: [
        { label: 'About SalesClaw', click: () => void showAbout() },
        { type: 'separator' },
        { role: 'quit' },
      ],
    },
    viewMenu,
    {
      label: 'Help',
      submenu: [
        { label: 'SalesClaw on GitHub', click: () => void openExternalUrl('https://github.com/Xx-173/salesclaw') },
      ],
    },
  ];
