...
export function MainAgentCapabilitiesSection() {
  ...
  const persistedHostPolicy = profile
    ...
    : null;
  const managedSkillsError = skillSelectionError(' SalesClaw Skill', {
    mode: skillsMode,
    ids: skillIds,
  });
  ...
  const save = async () => {
    ...
    try {
      ...
      await loadProfiles();
      toast.success('主 SalesClaw 模型、推理档位与能力已保存');
    } catch (error) {
    ...
    <section className="space-y-5 border-b border-border py-6">
      <div>
        ...
        <p className="mt-1 text-xs leading-5 text-muted-foreground">
          为主智能体选择完整模型网关环境，并按来源控制 Skills 与 SalesClaw 附加的
      ...
      <div className="max-w-xl space-y-2">
        ...
        <Select value={modelConfigId} onValueChange={setModelConfigId}>
          <SelectTrigger aria-label="主 SalesClaw 模型配置">
      ...
      <div className="max-w-xl space-y-2">
        ...
        >
          <SelectTrigger aria-label="主 SalesClaw 推理努力档位">
      ...
      <div className="max-w-xl border-t border-border pt-5">
        <CapabilityPicker
          ...
          disabledLabel="关闭 SalesClaw MCP"
        >
          {mcpMode === 'custom' && (
