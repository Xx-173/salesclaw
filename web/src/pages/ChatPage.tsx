  ...
  return (
    <div className="h-full flex bg-muted/30">
        ...
        <div className="block lg:hidden w-full overflow-y-auto">
          ...
          <div className="flex items-center gap-3 px-4 pt-5 pb-3">
            <img
              ...
              alt={appearance?.appName || 'SalesClaw'}
              className="h-8"
            />
        ...
        <div className="hidden lg:flex flex-1 items-center justify-center bg-background rounded-t-3xl rounded-b-none mt-5 mr-5 mb-0 ml-3 relative">
          <div className="text-center max-w-sm">
            ...
            <h2 className="text-xl font-semibold text-foreground mb-2">
              欢迎使用 {appearance?.appName || 'SalesClaw'}
            </h2>
      ...
      <ConfirmDialog
        ...
        title="重建工作区"
        message={`确认重建工作区「${clearState.name}」吗？这会永久删除全部聊天记录、上下文、所有子对话及其消息、工作目录文件，以及该工作区的全部 Memory（含版本历史、遗忘记录和 SalesClaw 称呼偏好）；Home 还会重置首次唤醒状态。关联定时任务会停止并移入回收站，运行历史保留；持久化目录 (data/extra/) 保留。此操作不可撤销。`}
        confirmText="确认重建"
        cancelText="取消"
