    ...
    <div className="min-h-full bg-background">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-background border-b border-border px-6 py-4">
          <PageHeader
            title="Skills"
            subtitle={`我的 ${skills.filter((item) => item.source === 'user').length} · SalesClaw 内置 ${skills.filter((item) => item.source === 'project').length} · 宿主机 ${skills.filter((item) => item.source === 'external').length} · 启用 ${enabledCount}`}
            actions={
        ...
        <div className="mx-6 mt-4 rounded-lg bg-muted px-4 py-3 text-xs leading-5 text-muted-foreground">
          “我的 Skills”可安装和管理；SalesClaw 内置与宿主机 Skills 只读。智能体
              ...
              {loading && skills.length === 0 ? (
                ...
                <>
                  ...
                  {projectSkills.length > 0 && (
                    <div>
                      <h2 className="text-sm font-semibold text-muted-foreground mb-3">
                        SalesClaw 内置 ({projectSkills.length})
                      </h2>
                      <div className="space-y-2">
